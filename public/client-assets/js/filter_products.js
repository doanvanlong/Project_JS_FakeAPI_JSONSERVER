...
if (url == "ao_nam.html") {
  ...
  viewPageQuanNam();
} else if (url == "product_love.html") {
  // hiển thị trang sản phẩm  áo nam
  async function viewPageProductLove() {
    if (localStorage.getItem("product_love")) {
      let products = JSON.parse(localStorage.getItem("product_love"));
    
      $(".countProductSale").innerText = products.length + " Sản phẩm";
      //khi select lọc sản phẩm all
      function sortProductAll() {
      ...
      function renderProductSale() {
        let renderProductLove = products.map(function (val) {
          let price = new Number(val.price);
          price = price.toLocaleString("vi", {
            style: "currency",
            currency: "VND",
        ...
        renderProductLove = renderProductLove.join("");
        $("#list_product_sale").innerHTML = renderProductLove;
        $('.del-product_love').onclick = function(){
          localStorage.removeItem("product_love");
         location.reload()
        }
      }
      renderProductSale()
